...
function check_domain($options) {
    ...
    //$cmd = 'whois '.$domain.' '. $whois_server .' | grep -i \'expir\|renew\|paid-till\''; //Original
	 //$cmd = 'whois '.$domain.' '. $whois_server .' | grep -v "clientRenewProhibited" | grep -i \'expir\|renew\|paid-till\''; V2
	 $cmd = 'whois '.$domain.' '. $whois_server .' | grep -v "PLEASE-RENEW-YOUR-DOMAIN-IN-WWW.22.CN" | grep -v "clientRenewProhibited" | grep -i \'expir\|renew\|paid-till\'';
